      ...
      {services.length > 0 && (
        <ul className="space-y-1">
          {services.slice(0, 3).map((service, index) => (
            <li
              key={index}
              className="text-sm text-olive-500 flex items-center gap-2"
            >
...
export const TeamCard = ({ name, designation, image }) => {
  ...
  useLayoutEffect(() => {
    ...
    fitNameInOneLine();
    window.addEventListener("resize", fitNameInOneLine);
    return () => window.removeEventListener("resize", fitNameInOneLine);
  }, [name]);
      ...
      <div className="relative z-10 flex flex-col items-center pt-6 pb-6 px-3 h-full">
        ...
        <div className="w-full flex justify-between items-center mb-4 pl-1 pr-1">
          ...
          <div className="flex flex-col items-end">
            <h3 className="text-gold-200 font-serif font-bold tracking-[0.15em] text-xs uppercase">
              Law Firm
            </h3>
        ...
        <h2
          ref={nameRef}
          className="w-full px-2 font-serif font-bold text-white tracking-[0.06em] uppercase mb-1 text-center drop-shadow-md leading-tight whitespace-nowrap"
        >
          {name}
        </h2>
...
export const LocationCard = ({
  ...
  return (
    <div
      className={`group bg-white rounded-xl border border-olive-100 hover:border-gold-300 hover:shadow-xl transition-all duration-300 overflow-hidden ${isHeadOffice ? "border-gold-400 shadow-lg" : ""}`}
    >
      {/* Card Content */}
      <div className="p-5">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="w-10 h-10 bg-olive-800 group-hover:bg-gold-500 rounded-lg flex items-center justify-center transition-colors duration-300">
            <svg
              className="w-5 h-5 text-gold-400 group-hover:text-white transition-colors"
              fill="currentColor"
              viewBox="0 0 20 20"
            >
              <path
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden">
      ...
      <div className="p-6">
        ...
        <div className="text-olive-600 text-sm">{children}</div>
      </div>
    </div>
  );
};
